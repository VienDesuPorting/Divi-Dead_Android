...
void text_at_base(SDL_Color c_front, SDL_Color c_back, int x, int y, char *buffer, int update) {
        ...
        surface = TTF_RenderUTF8_Shaded(font2, buffer, c_front, c_back);
...
int main(int argc, char* argv[]) 
{
...
#ifdef __ANDROID__
        /* On Android, pl_mpeg only supports MPEG-1 .MPG files. AVI/ROQ
         * fallbacks removed — users should convert OPEN.AVI to OPEN.MPG
         * (see populate_assets.sh and README). */
        ...
        if (!MOVIE_PLAY(opening_path, 1)) {
                printf("OPEN.MPG not found or playback failed\n");
        }
#else
